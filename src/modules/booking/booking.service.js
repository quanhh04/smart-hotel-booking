...
const getUserBookings = async (userId) => {
  return bookingModel.getBookingsByUserId(userId);
};

const cancelBooking = async ({ bookingId, userId }) => {
  const booking = await bookingModel.cancelBooking({ bookingId, userId });
  if (booking) notificationService.notifyBookingCancelled(booking);
  return booking;
};

const getBookingDetail = async (bookingId, userId) => {
  const booking = await bookingModel.getBookingById(bookingId);
  if (!booking) throw createError('Không tìm thấy đặt phòng', 404);
  if (booking.user_id !== userId) throw createError('Bạn không có quyền thực hiện thao tác này', 403);
  return booking;
};
