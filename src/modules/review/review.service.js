...
const createReview = async ({ userId, bookingId, rating, comment }) => {
  ...
  if (!booking) throw createError('Không tìm thấy booking', 404);
  if (Number(booking.user_id) !== Number(userId)) throw createError('Bạn không có quyền đánh giá booking này', 403);
  if (booking.status !== 'PAID' || new Date(booking.check_out) >= new Date()) {
    throw createError('Booking chưa đủ điều kiện để đánh giá', 400);
  }

  const existing = await reviewModel.getExistingReview(bookingId);
  if (existing) throw createError('Booking này đã được đánh giá', 409);

  const review = await reviewModel.createReview({ bookingId, userId, hotelId: booking.hotel_id, rating, comment });
...
const updateReview = async ({ reviewId, userId, rating, comment }) => {
  ...
  if (Number(review.user_id) !== Number(userId)) throw createError('Bạn không có quyền thực hiện thao tác này', 403);
  return reviewModel.updateReview({ reviewId, rating, comment, hotelId: review.hotel_id });
};
...
const deleteReview = async ({ reviewId, userId }) => {
  ...
  if (!review) throw createError('Không tìm thấy đánh giá', 404);
  if (Number(review.user_id) !== Number(userId)) throw createError('Bạn không có quyền thực hiện thao tác này', 403);
  return reviewModel.deleteReview({ reviewId, hotelId: review.hotel_id });
};
