...
const searchHotels = async ({ keyword, minPrice, maxPrice, stars, sortBy, sortOrder, page, limit }) => {
  ...
  if (keyword !== undefined) {
    values.push('%' + keyword + '%');
    idx++;
    filters.push(`(unaccent(h.name) ILIKE unaccent($${idx}) OR unaccent(h.address) ILIKE unaccent($${idx}))`);
  }
  ...
  const query = `
    SELECT h.id, h.name, h.address, h.description, h.rating, h.reviews,
           h.price_from, h.stars, h.created_at,
           COALESCE(
             (SELECT jsonb_agg(si.url ORDER BY hi.sort_order)
              FROM hotel.hotel_images hi
              JOIN settings.images si ON si.id = hi.image_id
...
const getHotelDetailById = async (hotelId) => {
  const result = await pool.query(
    `SELECT h.id, h.name, h.address, h.description, h.created_at,
            h.rating, h.reviews, h.price_from, h.stars,
            COALESCE(
              (SELECT jsonb_agg(si.url ORDER BY hi.sort_order)
               FROM hotel.hotel_images hi
               JOIN settings.images si ON si.id = hi.image_id
...
const hasActiveBookings = async (hotelId) => {
  const result = await pool.query(
    `SELECT COUNT(*)::int AS count
     ...
     WHERE rt.hotel_id = $1 AND b.status IN ('PENDING', 'CONFIRMED', 'PAID')`,
    [hotelId],
  );
...
const deleteHotel = async (hotelId) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM hotel.room_amenities WHERE room_type_id IN (SELECT id FROM hotel.room_types WHERE hotel_id = $1)`, [hotelId]);
    await client.query(`DELETE FROM hotel.room_types WHERE hotel_id = $1`, [hotelId]);
    await client.query(`DELETE FROM hotel.hotels WHERE id = $1`, [hotelId]);
...
const getHotelById = async (hotelId) => {
  const result = await pool.query('SELECT id FROM hotel.hotels WHERE id = $1', [hotelId]);
  return result.rows[0] || null;
};
...
const getRoomsByHotelId = async ({ hotelId, page = 1, limit = 10 }) => {
  ...
  const result = await pool.query(
    `SELECT r.id, r.hotel_id, r.name, r.price_per_night, r.max_guests,
            r.description, r.bed, r.size, r.total_quantity, r.created_at,
...
module.exports = {
  searchHotels, createHotel, getHotelDetailById, updateHotel,
  hasActiveBookings, deleteHotel, getHotelById, getRoomsByHotelId,
};
